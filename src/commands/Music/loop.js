    ...
    run: async (client, msg, args) => {
        ...
        if(!option || option.toLowerCase() === "song") {
            if(player.trackRepeat === false) {
                ...
                msg.channel.send(`${client.Emojis.check} I am no longer looping **the current track**`);
            };
        } else if(option.toLowerCase() === "queue") {
            if(player.queueRepeat === false) {
                player.setQueueRepeat(true);
                msg.channel.send(`${client.Emojis.check} I am now looping **the entire queue**`);
            } else {
                player.setQueueRepeat(false);
                msg.channel.send(`${client.Emojis.check} I am no longer looping **the entire queue**`);
            };
        } else return client.Errors.invalidArgs(msg, "loop");
    }
};